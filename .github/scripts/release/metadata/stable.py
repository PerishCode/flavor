...
USER_AGENT = "flavor-release-stable/1.0"
...
def read_cargo_version() -> str:
    cargo_toml = Path("crates/flavor-cli/Cargo.toml")
...
def next_stable(cargo_version: str) -> tuple[str, str, str]:
    public_url = os.environ.get("FLAVOR_RELEASES_PUBLIC_URL", "").rstrip("/")
    metadata_url = os.environ.get("FLAVOR_STABLE_METADATA_URL")
    if not metadata_url:
        if not public_url:
            fail("FLAVOR_RELEASES_PUBLIC_URL is required")
    ...
    if ordering == 0:
        fail(
            ...
            f"crates/flavor-cli/Cargo.toml before re-running"
        )
