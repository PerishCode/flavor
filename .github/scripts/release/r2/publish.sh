...
for name in FLAVOR_RELEASES_S3_AK FLAVOR_RELEASES_S3_SK FLAVOR_RELEASES_S3_BUCKET FLAVOR_RELEASES_S3_URL FLAVOR_RELEASES_PUBLIC_URL RELEASE_CHANNEL RELEASE_VERSION RELEASE_ROOT GITHUB_OUTPUT GITHUB_REPOSITORY GITHUB_SHA GITHUB_RUN_ID GITHUB_RUN_ATTEMPT GITHUB_WORKFLOW; do
  if [ -z "${!name:-}" ]; then
...
for file_path in "$release_root"/flavor-*.tar.gz "$release_root"/flavor-*.zip "$release_root"/checksums.txt; do
...
metadata = {
    ...
    "artifacts": {
        ...
        "macArm64": artifact("flavor-aarch64-apple-darwin.tar.gz", "application/gzip"),
        "macX64": artifact("flavor-x86_64-apple-darwin.tar.gz", "application/gzip"),
        "winX64": artifact("flavor-x86_64-pc-windows-msvc.zip", "application/zip"),
