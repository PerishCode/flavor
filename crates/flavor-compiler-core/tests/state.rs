use flavor_compiler_core::{CompilerCoreConfig, CompilerCoreState, Token, TokenCursor};

#[test]
fn config_is_state_injected() {
    let mut config = CompilerCoreConfig::default();
    config.snapshot.include_trivia = false;
    let state = CompilerCoreState::new(config);
