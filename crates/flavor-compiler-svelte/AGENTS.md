...
`crates/flavor-compiler-svelte/` owns AST-only Svelte compiler substrate on top
of `flavor-compiler-core`, with embedded expression validation delegated to
`flavor-compiler-ts`.
...
- `src/state/` owns `SvelteCompilerConfig` and `SvelteCompilerState`.
...
```bash
cargo test --locked -p flavor-compiler-svelte
```
...
- Keep TypeScript/JavaScript expression parsing in `flavor-compiler-ts` and map
