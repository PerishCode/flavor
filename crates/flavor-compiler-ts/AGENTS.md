...
`crates/flavor-compiler-ts/` owns TypeScript, JavaScript, and TSX syntax and
facts on top of `flavor-compiler-core`.
...
- `src/ast/`, `src/facts/`, and `src/visit/` own typed AST access, derived facts,
  and traversal hooks.
- `src/state/` owns `TsCompilerConfig` and `TsCompilerState`.
...
```bash
cargo test --locked -p flavor-compiler-ts
```
