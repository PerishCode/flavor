...
use flavor_compiler_core::{Diagnostic, SourceText, Span, SyntaxBuilder, SyntaxNode, Token};

use crate::{
    ast::TsSourceFile,
    state::{SourceMode, TsCompilerConfig},
...
impl<'a> Parser<'a> {
    ...
    fn error_at(&mut self, span: Span, message: &str) {
        self.diagnostics
            .push(Diagnostic::error(span, message.to_string()));
    }
}
