...
`crates/flavor-compiler-vue/` owns Vue SFC and template facts on top of
`flavor-compiler-core`, with embedded expression validation delegated to
`flavor-compiler-ts`.
...
- `src/state/` owns `VueCompilerConfig`, `TemplateConfig`, and
  `VueCompilerState`.
...
```bash
cargo test --locked -p flavor-compiler-vue
```
...
- When validating embedded expressions, keep TypeScript-specific parsing in
  `flavor-compiler-ts` and map diagnostics back to Vue source offsets.
...
`flavor-compiler-ts` where possible.
