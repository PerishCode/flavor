...
use flavor_compiler_vue::{
    run,
    template::{parse_template, VueTemplateKind},
    VueCompilerConfig,
};

fn has_node(ast: &flavor_compiler_vue::TemplateAst, kind: VueTemplateKind) -> bool {
    ast.syntax()
        .descendants()
        .any(|node| node.kind() == RawSyntaxKind::from(kind))
}

fn has_token(ast: &flavor_compiler_vue::TemplateAst, kind: VueTemplateKind) -> bool {
    ast.syntax()
