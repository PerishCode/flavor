...
impl SveltePluginAnalyzer {
    pub fn new(config: SveltePluginConfig) -> Self {
        Self {
            state: SveltePluginState::new(config),
        }
    }

    pub fn run(&mut self, source: SourceText) -> SvelteAnalysisOutput {
...
pub fn run(source: SourceText, config: SveltePluginConfig) -> SvelteAnalysisOutput {
    SveltePluginAnalyzer::new(config).run(source)
}
