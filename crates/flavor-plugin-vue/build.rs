...
fn main() {
    ...
    let grammar_dir = manifest_dir.join("../../grammars/vue");
    let lexer = grammar_dir.join("VueTemplateLexer.g4");
    let parser = grammar_dir.join("VueTemplateParser.g4");
    ...
    let schema = RawAstSchema::from_g4_sources("vue-template", 1000, &[parser, lexer]).unwrap();
    ...
    fs::write(out_dir.join("vue_template_kind.rs"), generated).unwrap();
}
