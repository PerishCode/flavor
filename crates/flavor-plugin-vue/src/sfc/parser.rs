...
pub struct VueSfcDescriptor {
    pub template: Option<VueSfcBlock>,
    pub script: Option<VueSfcBlock>,
    pub script_setup: Option<VueSfcBlock>,
    pub styles: Vec<VueSfcBlock>,
    pub custom_blocks: Vec<VueSfcBlock>,
    pub errors: Vec<VueSfcError>,
}
...
pub fn parse_sfc(source: &str) -> VueSfcDescriptor {
    ...
    while cursor < source.len() {
        ...
        if source[open_start..].starts_with("<!") || source[open_start..].starts_with("<?") {
            let Some(end) = find_open_tag_end(source, open_start + 2) else {
                descriptor.errors.push(error_at(
                    source,
                    open_start,
                    "unclosed top-level declaration",
                ));
                break;
            };
            cursor = end + 1;
        ...
        let (content_end, next_cursor) = if open_tag.self_closing {
            ...
            (close_tag.start, close_tag.end)
        };

        let block = VueSfcBlock {
            tag: open_tag.name,
...
fn push_block(
    descriptor: &mut VueSfcDescriptor,
    source: &str,
    open_start: usize,
    block: VueSfcBlock,
) {
    match block.tag.as_str() {
        ...
        "script" if block.attrs.contains_key("setup") => {
            if block.attrs.contains_key("src") {
                descriptor.errors.push(error_at(
                    source,
                    open_start,
                    "<script setup> cannot use src",
                ));
            }
            if descriptor.script_setup.is_some() {
                descriptor.errors.push(error_at(
                    source,
                    open_start,
                    "duplicate top-level <script setup> block",
                ));
            } else {
...
fn validate_descriptor(descriptor: &mut VueSfcDescriptor) {
    if descriptor.errors.is_empty()
        && descriptor.template.is_none()
        && descriptor.script.is_none()
        && descriptor.script_setup.is_none()
    {
        descriptor.errors.push(VueSfcError {
            line: 1,
            message: "at least one <template> or <script> block is required".to_string(),
        });
    }
    ...
    if descriptor.script_setup.is_some()
        && descriptor
            .script
            .as_ref()
            .is_some_and(|script| script.attrs.contains_key("src"))
    {
        let line = descriptor
            .script
            .as_ref()
            .map(|script| script.line)
            .unwrap_or(1);
        descriptor.errors.push(VueSfcError {
            line,
            message: "<script> cannot use src when <script setup> is also present".to_string(),
        });
    }
}
...
fn parse_open_tag(source: &str, open_start: usize) -> Result<Option<OpenTag>, String> {
    ...
    let raw_attrs = &source[name_end..open_end];
    let trimmed_attrs = raw_attrs.trim_end();
    let self_closing = trimmed_attrs.ends_with('/');
    let attrs_end = if self_closing {
        trimmed_attrs
            .rfind('/')
            .expect("self-closing attribute source contains slash")
    } else {
        raw_attrs.len()
    };
    ...
    Ok(Some(OpenTag {
        name,
        attrs: parse_attrs(&raw_attrs[..attrs_end]),
        end: open_end,
        self_closing,
    }))
}
...
fn error_at(source: &str, offset: usize, message: impl Into<String>) -> VueSfcError {
    VueSfcError {
